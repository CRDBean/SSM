...
public class MyShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		User user = userService2.selectByName(username);
		// 如果用户不存在，抛此异常
		if (null == user) {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		System.out.println("ShiroRealm  AuthorizationInfo:" + principal.toString());// 返回用户的用户名

		User user = userService2.selectByName(username);
		// 根据用户名来查询数据库赋予用户角色,权限（查数据库）
		Set<String> roles = new HashSet<>();
		Set<String> permissions = new HashSet<>();
		List<Role> roless=user.getRoles();
		for (Role role : roless) {
			roles.add(role.getName());
			List<Permission> permissionss=role.getPermissions();
